...
def get_response_from_lv_data(
    ...
    try:
        if use_curl:
            options = [f'{curlExePath}', '-G', '--data-urlencode', f"sql={sql}", API_BASE_URL]
            logger.debug(f"Using curl with options: {options}")
            rOutput = subprocess.check_output(options, shell=False, timeout=400, text=True)
            import json
            ...
            response = requests.get(API_BASE_URL, params=dict(sql=sql), timeout=DEFAULT_TIMEOUT)
...
def records_to_dataframe(
    ...
    """
    df=records_to_dataframe_raw(fields, records, columns=column_dict.values())
...
def get_lv_meteo_data(
    ...
    """
    try:
        sql = create_lv_meteo_data_sql(table, stations, parameters, startdate, enddate)
        data = get_response_from_lv_data(sql, use_curl=use_curl, curlExePath=curlExePath)
        ...
        if 'date' in df.columns and not df.empty:
            ...
            df = df.drop_duplicates(keep='first').set_index(['station_id', 'param', 'date']).sort_index()
...
if __name__=='__main__':
    parameters=['HTDRY','HATMN','HATMX']
    stations=['RIGASLU','RIDM99MS','RILP99PA']
    enddate=datetime.datetime.now()
    startdate=enddate - datetime.timedelta(days=30)
    
    df=get_lv_meteo_data(lv_meteo_tables.meteo_archive_avg.table_name, stations, parameters,startdate, enddate)
